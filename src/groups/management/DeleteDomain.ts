...
export class DeleteDomain implements Executor {
    async callback(ctx: Message, args: string[], ref: Blueprint<BaseConfig>) {
        ...
        try {
            ...
            return await dispatch_error_embed(ctx, `An error occurred whilst deleting the domain:\n**${e.message}**.`);
        }
        return await dispatch_success_embed(ctx, `Successfully deleted domain **${args[0]}**.`);
    }
}